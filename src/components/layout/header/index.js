  ...
  return (
    <div className="bg-indigo-950">
            ...
            {arrMenu.map((item, index) => (
              <div key={index} className="relative group">
                <Link href={item.sub ? "#" : item.link} legacyBehavior>
                  <a 
                    className="block ml-4 px-3 py-2 rounded-md text-sm font-medium text-gray-300 hover:text-white hover:bg-gray-700 focus:outline-none focus:text-white focus:bg-green-900 transition duration-150 ease-in-out"
                ...
                {item.sub && item.sub.length > 0 && (
                  <div className={`absolute left-0 w-48 mt-2 py-2 bg-white rounded-md shadow-xl z-20 ${activeMenu === item.title ? 'block' : 'hidden'}`}>
      ...
      <div className={'lg:hidden'}>
        <div className={`px-2 pt-2 pb-3 ${activeSideMenu ? 'block' : 'hidden'}`}>
        {arrMenu.map((item, index) => (
          <div key={index} className="relative group">
            <Link href={item.sub ? "#" : item.link} legacyBehavior>
              <a 
                className="block ml-4 py-2 rounded-md text-sm font-medium text-gray-300 hover:text-white hover:bg-gray-700 focus:outline-none focus:text-white focus:bg-green-900 transition duration-150 ease-in-out"
                onClick={() => handleMenu(item.title)}
              >
                {item.title}
              </a>
