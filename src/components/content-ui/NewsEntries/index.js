...
export default function NewsEntries({ entries }) {
    ...
    <>
        <div className='flex flex-row w-full items-center justify-start mt-8'>
            <h1 className="text-2xl lg:text-5xl font-extrabold col-span-3 row-span-1 mt-4 mb-2 px-1">
        ...
        <div className="flex flex-col p-2 lg:grid lg:grid-cols-3 lg:grid-rows-2 lg:gap-2 w-full h-auto mt-2">
            ...
            <div>
                <button className="bg-green-700 text-white hover:bg-slate-800 font-bold py-2 px-4 rounded-full mt-4 lg:mt-1">
                    <Link href="/thi-truong/thi-truong">{locale === "en-US" ? "View More" : "Xem Thêm"}</Link>
                </button>
            </div>
        </div>
    </>
